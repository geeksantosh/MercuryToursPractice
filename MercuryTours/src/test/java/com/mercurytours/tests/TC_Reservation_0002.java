...
public class TC_Reservation_0002 extends BaseClass {
	...
	public void flightReservation() throws Exception {
		 ...
		 * loginpage.loginUsername(username); loginpage.loginPassword(password);
		 * loginpage.loginBtn();
		 */
		Thread.sleep(3000);
		...
		if(expectedURL.equals(actualURL)) {
			...
			System.out.println("DEPART and RETURN data submitted correctly and openend new page for flight selection New URL is "+actualURL);
		}else {
			captureScreen(driver, "flightReservation"); 
			Assert.assertFalse(false);
			System.out.println("DEPART and RETURN data couldnt be submitted. Because of form details are not completed. URl after test failed: "+ actualURL);
		}
	}
}
